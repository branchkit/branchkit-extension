...
**OPEN (2026-05-30): a distinct *nav-time* wedge — reproduces in the driver, but real Firefox is clean.** See the "Nav-time wedge" section below. In the A/B driver a same-document SPA nav (clicking a YouTube recommendation, /watch → /watch) wedges the renderer so hard Firefox's debugging protocol stops responding, while an extension-off control on the *same* nav sails through — so under harness stress the cost is the extension's, not YouTube's. **Re-confirmed deterministically 2026-05-30 after `8c0e368`** (which fixed only the scroll-soak path, not this one). **But** the user's *real* Firefox navs in the same window completed cleanly (`cs_nav_step` breadcrumbs all fire on schedule, no wedge), matching their lived experience — so this is "reproduces under harness stress, low real-world impact," not a confirmed production regression. Attribution is still blocked because the driver wedges too hard to ship any breadcrumb/perf line; pinning the step needs a *real-Firefox* repro. This is the current open code lead.
...
**Re-confirmed 2026-05-30 (deterministic, after `8c0e368`).** Re-ran the A/B driver on the current build (which includes `8c0e368`'s full-page-swap discovery slicing) to check whether the scroll-soak fix also closed this. It did not — the nav-time wedge is a *separate* path:
...
**Important divergence — driver wedges, real Firefox does not.** During the same window the user's *real* Firefox did several /watch SPA navs, and their `cs_nav_step` breadcrumbs in `actuator.log` show the from_cache path **completing cleanly every time**: `drop_disconnected` + `sync_now` at `at_ms:0`, then `deferred_scan` firing on schedule at `at_ms:~303`. No wedge — matching the user's lived experience that nav "feels fine now." Only the Playwright-driven Firefox wedges. Candidate reasons the harness amplifies it: cold fresh profile (no warm HTTP/media cache), Playwright/juggler instrumentation overhead, and a constrained single-tab context — none of which the real browser hits. This does **not** clear the path (the driver proves the extension *can* wedge a /watch nav), but it downgrades it from "confirmed production regression" to "reproduces under harness stress; real-world impact currently low."

**Attribution still blocked — the driver can't self-report, and every readback channel dies with the wedge.** The driver's video (`qQDrqV5Hw4c`) produced **zero** usable lines in the perf trail and `actuator.log` — it wedges too hard to ship anything, exactly as the original note predicted.

Two things learned 2026-05-30 while trying to attribute it anyway:

1. **The driver is Firefox *Nightly* (`org.mozilla.nightly`), a different browser bundle from the user's daily Firefox (`org.mozilla.firefox`).** Playwright's bundled Firefox is Nightly. The focus gate confirms it: while the driver window held OS focus, the plugin logged `grammar batch REJECTED inactive_browser bundle="org.mozilla.firefox" focused="org.mozilla.nightly"` — i.e. it rejected the user's *real* Firefox because the driver's Nightly was the focused browser. So (a) the clean `cs_nav_step` breadcrumbs cited above are genuinely the stable browser, not the driver, and (b) the driver/Nightly harness reproduces the wedge on `/watch → /watch` while the same nav on stable Firefox completes cleanly.

   **2026-05-30 user-side repro on stable Firefox (revises this attribution).** A wild repro landed on stable Firefox `org.mozilla.firefox`, in the user's normal session, on a YouTube channel `/@MaxandOccy/videos` page (clicked the Videos tab via voice). `actuator.log` at 02:39:04 UTC shows `drop_disconnected` and `sync_now` at `at_ms:0-1` (clean), then **`deferred_scan` never fires** — the breadcrumb that should land at `at_ms:~300-700` is absent. Compare the prior nav at 02:38:50 (watch → `/@MaxandOccy` channel home), which logged `deferred_scan` at `at_ms:677, 721, 723, 726` and completed normally. So stable Firefox is **not** immune; the wedge fires on heavy SPA targets specifically — the channel `/videos` page (paginated thumbnail grid) wedges where the channel home does not. Trigger appears to be page-weight, not browser build. The Nightly-only framing on commit `4b0286e` is too narrow.

2. **The recovery-read trick does not work.** I extended the driver to wait ≤90s post-wedge and read the page's own `dataset.branchkitPerf` CPU buckets (which survive in the page even when it can't ship them). It hung without printing — once the wedge kills Firefox's juggler pipe, *every* Playwright channel dies, DOM reads included. There is no read path through Playwright that outlives this wedge.

**So attribution needs a side-channel that leaves the wedged renderer.** Two viable options, both real work: (a) add a dev-only ring buffer in the background service worker that retains the last N `cs_nav_step`/perf messages, then read it from the SW's *own* Playwright target — a separate juggler target that survives a content-main-thread wedge; or (b) run the driver with the Nightly window genuinely OS-focused so the existing `cs_nav_step`/perf POSTs reach the plugin and land in `actuator.log` despite the content wedge. Until one of those lands, **which** step wedges is unproven — and note the leading hypothesis (from_cache teardown+rebuild) is itself unconfirmed: the wedge may instead be the connection-independent baseline path (MutationObserver firehose + discovery + badge teardown/re-layout on YouTube's full-page DOM swap), which runs regardless of whether the plugin delivered a rescan. Do not build the fix until one of these channels pins the step.
...
- `dropDisconnectedWrappers` (`content.ts`) now emits `recordCpu('dropDisconnectedWrappers', …)` + a `:size:` bucket. On a full DOM swap *every* wrapper disconnects at once, so if this step is the cost it'll now surface in `topLabels` instead of hiding in `unattributedMs`.
- `rescanForNav`'s from_cache branch now posts `pipeline.cs_nav_step` breadcrumbs around every heavy step — both `:start` and `:end` (`drop_disconnected:start/end` → `sync_now:start/end` → `deferred_scan:start/end`), each with `at_ms` relative to rescan entry — to the service worker. The SW never wedges, so even a hard freeze leaves a trail naming the last step *entered*; the missing matching `:end` then pins the body that killed the main thread. (Originally only `:start` was logged; revised 2026-05-30 after the user's stable-Firefox repro on `/@MaxandOccy/videos` showed `sync_now` start firing but `deferred_scan` start never firing — i.e. the wedge sat *between* them, but without `:end` breadcrumbs we couldn't tell whether `dropDisconnectedWrappers` or `syncNow`'s body was the killer, or whether YouTube was hogging the thread between our sync_now-end and our setTimeout-fire.) (`syncNow` itself is left unlabeled for CPU on purpose: it's mostly `await postBatch` network time, so a wall-clock mark would be misleading; the START/END breadcrumb pair captures its real wall cost.)

**Repro to pin it:** run `scripts/_drive-firefox-nav-ab.mjs` (extension-on arm) after a build, do the /watch → /watch nav, then read the trail: the last `cs_nav_step` before the watchdog stall names the step it died in; if a perf snapshot ships on recovery, its `watchdogStalls[].topLabels` now distinguishes `dropDisconnectedWrappers` / `processScanBatch:syncA|syncB` (the walk) / `moCallback`+`processMutations`+`drainDiscovery` (the firehose drain) from `unattributedMs` (browser reflow of our badge DOM, i.e. re-layout). high `unattributedMs` with low tracked ⇒ badge re-layout / browser paint; high `doScanBatched`/`syncA|syncB` ⇒ the walk; high `moCallback`/`drainDiscovery` ⇒ the firehose.

**Leading hypothesis (not yet measured — don't build the fix until the repro above confirms).** `scheduleSpaRescan` hardcodes `from_cache:'true'`, a path designed for app-*refocus* where the DOM is unchanged. On a genuine content swap it does the worst of both: a full teardown (every wrapper disconnects → dropped → ~200 deletes synced) *and* a full rebuild (the deferred `doScanBatched` at +300ms), stacked and timed to land in the middle of YouTube's new-page render + the giant DOM-swap mutation burst. If confirmed, candidate fixes (cheapest first): push the reconciliation walk to `requestIdleCallback`/a longer adaptive delay so it stops racing the render burst; gate the walk on the mutation firehose settling into a quiet window; or have the content script distinguish "URL changed, DOM same" (skip teardown) from "URL changed, DOM swapped" (idle-scheduled rebuild).

**Fix landed 2026-05-30 (option 1 from the candidate list): idle-schedule the entire from_cache rescan body.** `rescanForNav` now wraps its from_cache branch in `requestIdleCallback({ timeout: 2000 })` (fallback `setTimeout(100)` for non-supporting browsers) and emits an `idle_fired` breadcrumb when the callback runs. Rationale: the synchronous teardown + grammar sync were landing on the main thread in the middle of YouTube's full-page DOM swap, where every `isConnected` check fights an in-progress reflow and the postBatch competes with the page's own work. Waiting one idle window lets the page settle before we touch anything. Cost is cosmetic — badges may stay painted on disconnected DOM during the wait, but the user just navigated away from them.

**What to verify on next repro (post-fix):**
- If `idle_fired` lands within ~50-200ms of the rescan dispatch AND all `:start`/`:end` pairs complete cleanly, the wedge is gone — YouTube's reflow was the killer and getting out of its way fixed it.
- If `idle_fired` doesn't fire for >1500ms (close to the 2s timeout), the browser is so busy that idle never comes; consider dropping the timeout or gating on `MutationObserver` quiescence instead.
- If `idle_fired` fires promptly but a `:start` still lacks its matching `:end`, the wedge is in OUR code, not YouTube's reflow — read which step to know whether to slice `dropDisconnectedWrappers` (the most likely body) or chunk `syncNow`'s payload.

**Fix outcome (2026-05-30, user repro on stable Firefox @ `/@MaxandOccy/videos`):** the nav-path itself is fixed — `idle_fired` landed at `at_ms:12`, all three frames completed `drop_disconnected:start/end` + `sync_now:start/end` cleanly in 12-13ms. But the Firefox slow-extension warning still fires, and `deferred_scan:start`/`:end` **never arrive** for that nav. That puts the surviving wedge in the ~300ms window between `sync_now:end` and the deferred-scan setTimeout firing — i.e. the steady-state MutationObserver firehose absorbing YouTube's full-page DOM swap (`moCallback` → `processMutations` / HUGE_MUTATIONS short-circuit → `drainDiscovery`), not the nav-path code we just hardened.

**Firehose breadcrumbs added 2026-05-30** (`content.ts` `firehoseStep()`, tag `pipeline.cs_firehose_step`). Threshold-gated by payload size (≥100 records / roots) so steady-state churn doesn't add 180 sendMessages/sec just for telemetry. Steps to expect on the next wedge:

| Step | Where | What a missing `:end` means |
|---|---|---|
| `moCallback:start` / `moCallback:filter_start` / `moCallback:filter_end` | top of MO callback, around the `records.filter(isOwnMutation)` loop | If `filter_end` is missing, the synchronous filter over a giant `records[]` is the killer |
| `moCallback:end_huge_scheduled` | when foreign≥1000 (HUGE_MUTATIONS short-circuit) | If missing after `filter_end`, the setTimeout schedule path itself blocked (unlikely) |
| `huge_path:timer_fired` / `huge_path:batched_start` / `huge_path:batched_end` | inside the HUGE_MUTATIONS setTimeout | If `timer_fired` missing, the setTimeout never ran (main thread wedged on something else). If `batched_end` missing, `discoverInSubtreeBatched` is wedging despite the 8c0e368 slicing |
| `moCallback:end_normal` | non-HUGE path, after `processMutations` returns | Missing pinpoints a wedge inside `processMutations` or `scheduleDeferredReposition` |
| `processMutations:start` / `processMutations:end` | wraps the record loop + `dropDisconnectedWrappers` | Missing `:end` means the synchronous walk over many removals (calling `dropDisconnectedWrappers`) is the killer |
| `drainDiscovery:start` / `drainDiscovery:end` | RAF-driven discovery drain | Missing `:end` despite the 8ms budget = a single root's `discoverInSubtree` blew the budget by itself (the "always one root per pass" safety valve becomes the trap on a heavy root) |

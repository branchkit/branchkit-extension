...
**Attribution still blocked — the driver can't self-report.** The driver's video (`qQDrqV5Hw4c`) produced **zero** lines in both the perf trail and `actuator.log` (`cs_nav_step` / perf snapshots) — it wedges too hard to ship anything, exactly as the original note predicted. So the driver confirms *that* it wedges but not *which* step. The tonight-added `cs_nav_step` breadcrumbs + `dropDisconnectedWrappers` label only pay off on a **real-Firefox** repro that wedges while breadcrumbs are still flowing. Next concrete step is therefore user-driven, not driver-driven: get a real-Firefox /watch nav to wedge (heavy page, many tabs, real cadence) and read the last `cs_nav_step` before the stall + any recovery snapshot's `topLabels`. Until then, do not build the fix.
...
- `rescanForNav`'s from_cache branch now posts `pipeline.cs_nav_step` breadcrumbs (`drop_disconnected` → `sync_now` → `deferred_scan`, each with `at_ms` relative to rescan entry) to the service worker *before* each heavy step. The SW never wedges, so even a hard freeze leaves a trail of the last step entered + inter-step gaps — unlike the post-completion `cs_scan_completed`, which never ships when the thread freezes. (`syncNow` itself is left unlabeled for CPU on purpose: it's mostly `await postBatch` network time, so a wall-clock mark would be misleading; the breadcrumb gap on either side of it captures its real wall cost.)

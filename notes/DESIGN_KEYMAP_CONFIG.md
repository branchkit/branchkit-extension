...
**3c — fold the hide chord into the keymap (Phase 3, with storage).** Once `keymap-storage.ts` exists, `ctrl+KeyF → toggle_hints` becomes a normal rebindable keymap entry and the standalone `hintHideKey` config + `getHintHideKey()` + the curated dropdown are deleted. Deferred to the storage phase deliberately: folding it earlier would regress the *currently rebindable* hide key to a static default until a store exists to back it.
...
- **Phase 3 — storage + editor:** `keymap-storage.ts` (mirror badge-settings/domain-rules, `chrome.storage.sync`); the options-page editor (rows, command dropdown, key-capture, schema-driven param controls, validation + always-mode warnings, reset); `onKeymapChanged` → `replaceAll`. Fold the hide chord into the keymap (§3c — delete the content.ts special-case + `getHintHideKey()`) now that a store backs a rebindable hide key. **First user-visible rebinding.**

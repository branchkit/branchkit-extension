...
**3c — fold the hide chord into the keymap (DONE, Phase 3).** `ctrl+KeyF → toggle_hints` is now a normal rebindable keymap entry; the standalone `hintHideKey` config (`config.ts`), the popup's key-capture UI (`popup.ts`/`popup.html`), and the dead `matchesCombo`/`isComboAllowed`/`DEFAULT_HIDE_KEY`/`HintHideKey` were deleted. The chord routes through the registry; to keep it toggling (and suppressing native Ctrl+F find) while typing in a field, the keyboard.ts insert-mode check now runs *after* real-modifier routing, so a bound chord fires in fields while unbound ones (Ctrl+A, Cmd+C) still fall through.

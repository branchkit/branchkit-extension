...
// Outbound chrome.runtime.sendMessage volume. Wraps the API at first
// snapshot read (lazy so test harnesses can install before the first
// message fires). Counts each call by `message.type` and accumulates an
// approximate byte size of the JSON-serialized payload. The actuator
// receives one HTTP POST per GRAMMAR_BATCH/CLAIM_LABELS, so this doubles
// as a proxy for outbound HTTP volume from the SW.
...
function ensureSendMessageWrapped(): void {
...
function resetMessageCounters(): void {
